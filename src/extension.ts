 ...
 *
 * Activation happens on `onStartupFinished`. The extension keeps the active
 * credential in `vscode.SecretStorage`, tracks daily claim state and usage in
 * `globalState`, reports everything through a right-aligned status bar item, and
 * exposes a small API so other extensions can report their token consumption.
 *
 * Command surface:
...
export async function activate(context: vscode.ExtensionContext): Promise<xTokenApi> {
...
function registerCommands(rt: Runtime): vscode.Disposable[] {
  return [
    register(rt, COMMANDS.fetchToken, () => commandFetchToken(rt)),
    register(rt, COMMANDS.selectProvider, () => commandSelectProvider(rt)),
    register(rt, COMMANDS.showToken, () => commandShowToken(rt)),
    register(rt, COMMANDS.clearToken, () => commandClearToken(rt)),
...
function createApi(rt: Runtime): xTokenApi {
  return {
    version: rt.version,
    providers: [...PROVIDERS],
...
async function commandSelectProvider(rt: Runtime): Promise<void> {
 ...
 */
async function commandFetchToken(
  rt: Runtime,
...
async function commandShowToken(rt: Runtime): Promise<void> {
...
async function commandSetKey(rt: Runtime, presetOverride?: ProviderPreset): Promise<void> {
  const config = readConfig()
...
async function commandClearToken(rt: Runtime): Promise<void> {
...
async function commandRotateKey(rt: Runtime): Promise<void> {
...
async function commandShowUsage(rt: Runtime): Promise<void> {
  const config = readConfig()
...
async function commandRefreshStatus(rt: Runtime): Promise<void> {
  const preset = getProvider(rt.state.activeProvider)
  if (preset?.quota)
...
async function refreshStatusBar(rt: Runtime, options: { forceQuota?: boolean } = {}): Promise<void> {
  const config = readConfig()
  const token = await rt.keys.getActiveToken()
  const keyCount = await rt.keys.getKeyCount()
  const preset = getProvider(rt.state.activeProvider)
...
async function verifyActiveKeyQuietly(rt: Runtime): Promise<void> {
  const config = readConfig()
  const token = await rt.keys.getActiveToken()
  const preset = getProvider(rt.state.activeProvider)
  if (!token || !preset)
